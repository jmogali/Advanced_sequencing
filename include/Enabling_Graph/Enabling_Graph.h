...
{
	private:
		std::vector<Enabling_Node> m_vec_nodes;
		inline void add_neigh(size_t uiIndex , size_t neigh) { return m_vec_nodes[uiIndex].add_neigh(neigh); };
		inline void add_robot(size_t uiIndex, size_t uiRobot) { return m_vec_nodes[uiIndex].add_robot(uiRobot); };
	...
	public:
		inline const std::vector<Enabling_Node>& get_Node_vec() {return m_vec_nodes;};
