...
class Greedy_Heuristic
{
	private:
		...
		Power_Set &m_power;
		std::vector<std::vector<size_t>> m_vec_node_depth_info;  // records the depth at which hole (node) first appeared (completed)
		...
		void initialize_node_depth_buffer(const std::vector<std::list<size_t>> &rob_seq);
